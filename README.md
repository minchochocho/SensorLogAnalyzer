...
> 현재는 CSV 파일 선택·검증·목록 표시, 정상·오류 집계, 거리·조도 상태 판정과 전체·유효·오류 탭 필터를 구현했습니다. 거리·조도 그래프는 사용자 지정 컨트롤과 임시 데이터 선 표시까지 구현했으며, CSV 데이터 연결과 결과 저장은 아직 남아 있습니다.
...
- [X]  거리·조도 그래프 컨트롤, 격자와 임시 데이터 선 구현 (빌드 확인)
- [ ]  CSV의 유효한 거리·조도 값을 그래프에 연결
...
- 거리 그래프는 최대 `400`, 조도 그래프는 최대 `255`를 임시 축 범위로 사용
- 거리에는 파란색, 조도에는 주황색 임시 데이터 선 적용

현재 `OnInitDialog()`에서 하드코딩한 6개 값을 이용해 그래프를 구성합니다. 그래프 클래스와 대화상자 연결 코드는 Debug x64 빌드에 성공했지만, 실제 선 표시 화면은 아직 문서에 확인 결과를 기록하지 않았습니다. 다음 단계에서 임시 값을 제거하고 CSV의 유효한 센서 값으로 교체합니다.
...
- Debug x64 명령줄 빌드 성공
- 다음 작업: 실행 화면의 임시 선 확인 후 CSV 유효 데이터를 두 그래프에 연결
