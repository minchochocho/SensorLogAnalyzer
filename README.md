...
> 현재는 CSV 파일 선택·검증·목록 표시, 정상·오류 집계, 거리·조도 상태 판정, 목록 필터, 시간축을 포함한 선 그래프, 판정 기준 변경과 분석 결과 CSV 저장을 구현했습니다. 기본 판정 기준의 화면 표시를 확인했으며, 변경 기준 적용·기본값 복원과 저장 결과의 실행 검증이 남아 있습니다.
...
- [X]  그래프 가로축의 처음·중간·마지막 측정 시각 표시 (개발자 실행 확인)
- [X]  경고 기준을 화면에서 변경하고 다시 판정하는 기능 (빌드 확인)
- [X]  분석 결과 CSV 저장 구현 (빌드 확인, 실행 검증 예정)
...
판정 함수 `GetDistanceStatus()`와 `GetLightStatus()` 및 목록의 4·5번 열 출력 연결을 소스에서 확인했습니다. 오류가 있는 행은 두 상태를 비워 두고 `오류 내용`만 표시합니다. `위험`과 `어두움`은 정상적으로 측정된 값의 상태이며 데이터 형식 오류에 포함하지 않습니다. 초기 경계값 `30`, `60`, `189`, `190`의 결과는 경계값 샘플과 실행 화면으로 확인했습니다.

분석 결과 저장은 구현했으며 실제 생성 파일 검증은 아직 수행하지 않았습니다.

### 판정 기준 변경

- 그래프 위의 `판정 기준` 영역에서 위험 거리, 주의 거리와 어두움 ADC 기준 입력
- 초기값은 위험 거리 `30cm`, 주의 거리 `60cm`, 어두움 기준 `190`
- 위험·주의 거리는 `0`보다 크고 `400` 이하인 유한한 숫자로 검증
- 위험 거리가 주의 거리보다 작은지 추가 검증
- 어두움 기준은 `0~255` 범위의 정수로 검증
- `기준 적용` 버튼을 누르면 이미 불러온 유효 레코드의 거리·밝기 상태를 다시 계산
- `기본값 복원` 버튼을 누르면 입력값과 내부 기준을 `30`, `60`, `190`으로 되돌리고 즉시 재판정
- 입력 오류가 있는 레코드는 기존 오류 내용과 빈 상태 열을 그대로 유지
- 변경된 상태는 목록 필터와 분석 결과 CSV 저장에도 반영

판정 기준 UI, 입력 검증과 재판정 코드는 Debug x64 빌드에 성공했습니다. 실행 화면에서 기본 기준 `30`, `60`, `190`이 입력란에 표시되고 경계값 샘플의 상태가 목록에 출력되는 것을 확인했습니다. 실제 입력값 변경과 기본값 복원에 따른 목록 상태 변화는 추가 확인이 필요합니다.
...
- `IDC_BUTTON_SAVE` 클릭 이벤트를 `OnBnClickedButtonSave()`에 연결
- 불러온 데이터가 없으면 먼저 CSV를 열도록 안내
- 저장 위치와 파일명을 고르는 `CFileDialog` 표시
- 현재 선택된 목록 탭과 관계없이 불러온 전체 레코드를 저장
- 행 번호, 시각, 거리, 조도, 거리 상태, 밝기 상태와 오류 내용을 7개 열로 출력
- 쉼표·따옴표·줄바꿈이 있는 필드는 따옴표로 감싸고 내부 따옴표를 두 번 기록
- 한글 상태와 오류 문구를 Excel에서 인식할 수 있도록 UTF-8 BOM 형식으로 저장
- 같은 이름의 파일이 있으면 덮어쓰기 확인 창 표시
- 파일 열기·쓰기 예외가 발생하면 오류 메시지를 표시하고 저장 중단

저장 헤더는 다음과 같습니다.

```csv
row,timestamp,distance_cm,light_adc,distance_status,light_status,error_message
```

저장 핸들러는 별도 출력 폴더를 사용한 Debug x64 빌드에 성공했습니다. 실행 중이던 기본 Debug 실행 파일은 잠겨 있어 같은 위치의 링크는 수행하지 못했으며, 실제 저장 대화상자와 생성된 CSV 내용은 개발자가 실행해 확인해야 합니다.
...
- **그래프 위:** 위험 거리·주의 거리·어두움 기준 설정 및 적용
...
- 화면 입력값으로 판정 기준을 변경하고 유효 레코드의 상태를 다시 계산
...
- [X]  실행 화면에서 처음·중간·마지막 시간 레이블 확인 및 캡처 추가
- [X]  분석 결과 저장 버튼 이벤트와 UTF-8 CSV 출력 코드 빌드
- [X]  판정 기준 입력 UI, 값 검증과 재판정 코드 빌드
- [X]  판정 기준 기본값 복원 버튼과 공통 재판정 함수 빌드
- [X]  기본 기준 `30`, `60`, `190`이 입력란에 표시되는지 확인 (개발자 실행 확인)
- [ ]  기준 변경 후 목록의 거리·밝기 상태가 즉시 변경되는지 확인
- [ ]  기본값 복원 후 입력값과 상태가 초기 기준으로 돌아오는지 확인
- [ ]  잘못된 범위와 `위험 거리 >= 주의 거리` 입력의 안내 문구 확인
- [ ]  파일을 열기 전에 저장 버튼을 눌렀을 때 안내 문구 확인
- [ ]  정상·오류 샘플의 분석 결과 저장 및 7개 출력 열 확인
- [ ]  저장 파일의 한글 상태·오류 문구와 덮어쓰기 확인
...
- `sensor_normal.csv`: `14:00:00`, `14:00:09`, `14:00:19` 표시

![](assets/20260911_115632_image.png)

- `sensor_errors.csv`: 오류 구간을 유지하면서 처음과 마지막 유효 시각 표시

![](assets/20260911_115640_image.png)

- `sensor_boundaries.csv`: 4개 레코드의 처음·중간·마지막 시각 표시

### 2026-09-11 — 분석 결과 CSV 저장

- `분석 결과 저장` 버튼 이벤트 연결
- 저장할 데이터가 없는 상태와 사용자의 저장 취소 처리
- 전체 `SensorRecord`를 7개 열의 결과 CSV로 내보내기
- CSV 특수문자 이스케이프와 UTF-8 변환 함수 추가
- Excel의 한글 인식을 위한 UTF-8 BOM 기록
- 파일 열기 및 쓰기 오류 메시지 처리
- Debug x64 전체 빌드 성공
- 다음 작업: 실제 저장 파일의 행 수·한글·오류 내용과 덮어쓰기 동작 확인

### 2026-09-11 — 화면 판정 기준 변경

- 그래프 위에 위험 거리·주의 거리·어두움 기준 입력란과 `기준 적용` 버튼 배치
- 기본값을 거리 `30cm`, `60cm`와 조도 ADC `190`으로 초기화
- 거리의 숫자 형식·허용 범위·두 기준의 순서 검증
- 조도 기준의 정수 형식과 `0~255` 범위 검증
- 기준 적용 시 유효한 `SensorRecord`의 거리·밝기 상태를 다시 계산
- 재판정 후 현재 목록 탭과 그래프를 갱신하고 저장 결과에도 새 상태 반영
- Debug x64 전체 빌드 성공
- 다음 작업: 입력 오류 메시지, 재판정 결과와 저장 파일을 실행 화면에서 확인

### 2026-09-11 — 판정 기준 기본값 복원

- `기본값 복원` 버튼을 판정 기준 영역에 추가
- 버튼 클릭 시 위험 거리 `30cm`, 주의 거리 `60cm`, 어두움 ADC `190`으로 복원
- 기준 적용과 기본값 복원이 공유하는 `RecalculateStatuses()` 함수로 재판정 코드 정리
- 복원 직후 현재 목록 탭과 그래프 갱신
- 별도 출력 폴더를 사용한 Debug x64 전체 빌드 성공
- 실행 중인 기본 Debug 프로그램이 기존 실행 파일을 잠그고 있어 해당 경로의 링크는 수행하지 않음

![](assets/20260911_133720_image.png)

판정 기준 입력란의 기본값 `30`, `60`, `190`과 경계값 샘플의 거리·밝기 상태가 표시된 실행 화면입니다. 이 캡처에는 `기본값 복원` 버튼이 보이지 않으므로, 복원 동작 확인은 체크리스트에 미완료로 남겨 두었습니다.
